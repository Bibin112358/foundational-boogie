  ...
  where "initial_state_local \<Omega> R \<Lambda> \<Lambda>' ns x = 
         (case (map_of (snd \<Lambda>') x) of Some tw =>
           (if (\<exists>z. R z = Some (Inl x)) then 
              ...
              Some (val_of_type (instantiate \<Omega> (fst tw)))
           )
...
lemma initial_state_lookup: 
  ...
  shows "initial_state_local \<Omega> R \<Lambda> \<Lambda>' ns y = lookup_var \<Lambda> ns x" (is "?u y = lookup_var \<Lambda> ns x")
proof -
...
lemma initial_state_global_lookup: 
  ...
  shows "initial_state_global \<Omega> R \<Lambda> \<Lambda>' ns y = lookup_var \<Lambda> ns x" (is "?u y = lookup_var \<Lambda> ns x")
proof -
  ...
  assumes 
       ...
       (* no shadowing *)
        ...
        shows "\<lparr>old_global_state = Map.empty, 
               ...
               local_state = (initial_state_local \<Omega> R \<Lambda> \<Lambda>' ns), 
               binder_state = Map.empty \<rparr> \<in> initial_set R \<Lambda> \<Lambda>' \<Omega> ns" (is "?u \<in> ?U")
proof (simp only: initial_set.simps, rule, intro conjI)
  show "state_typ_wf \<Omega> (local_state ?u) (snd \<Lambda>')"
  ...
  proof ((rule allI)+, rule impI)
    ...
    show "map_option type_of_val (local_state ?u y) = Some (instantiate \<Omega> \<tau>)"
    ...
    next
      case False
      hence "local_state ?u y = Some (val_of_type (instantiate \<Omega> \<tau>))"
  ...
  qed
next
  show "state_typ_wf \<Omega> (global_state ?u) (fst \<Lambda>')" using WellTyp GlobalsSame by simp
next
...
lemma init_state_dependent:"dependent \<Lambda>' \<Omega> (initial_set R \<Lambda> \<Lambda>' \<Omega> ns) ((rel_range R) \<union> set (map fst (fst \<Lambda>)))" 
  ...
  proof (rule ballI, rule allI, rule allI, rule impI, rule conjI[OF _ impI[OF allI[OF impI]]])
    fix u \<tau> d
    assume "u \<in> ?U"
    hence S1:"state_typ_wf \<Omega> (local_state u) (snd \<Lambda>')" and S2:"state_typ_wf \<Omega> (global_state u) (fst \<Lambda>')" by auto
    assume LookupTy:"lookup_var_ty \<Lambda>' d = Some \<tau>"
    thus "lookup_var_ty_match \<Lambda>' \<Omega> u d \<tau>"
      using state_typ_wf_lookup[OF S1 S2 LookupTy] 
      by (simp add: lookup_var_ty_match_def)
  next
    fix u \<tau> d v
    assume "u \<in> ?U"
    hence S1:"state_typ_wf \<Omega> (local_state u) (snd \<Lambda>')" and
          S2:"state_typ_wf \<Omega> (global_state u) (fst \<Lambda>')" and
    ...
    assume TypV:"type_of_val (v::('a, 'm) val) = instantiate \<Omega> \<tau>"
    have S1Upd:"state_typ_wf \<Omega> (local_state (update_var \<Lambda>' u d v)) (snd \<Lambda>')"
      unfolding state_typ_wf_def
    proof (rule allI, rule allI, rule impI)
      fix x \<tau>'
      assume MapOfX:"lookup_vdecls_ty (snd \<Lambda>') x = Some \<tau>'"
      show "map_option type_of_val (local_state (update_var \<Lambda>' u d v) x) = Some (instantiate \<Omega> \<tau>')"
      proof (cases "d = x")
        case True
        then show ?thesis 
      ...
      next
        case False
        from S1 MapOfX have "map_option type_of_val (local_state u x) = Some (instantiate \<Omega> \<tau>')"
          using state_typ_wf_def by blast        
        with False show ?thesis using local_state_update_other
          by metis
      qed
    qed
    have S2Upd:"state_typ_wf \<Omega> (global_state (update_var \<Lambda>' u d v)) (fst \<Lambda>')"
      unfolding state_typ_wf_def
    proof (rule allI, rule allI, rule impI)
      fix x \<tau>'
      assume MapOfX:"lookup_vdecls_ty (fst \<Lambda>') x = Some \<tau>'"
      from S2 MapOfX have GlobalUX:"map_option type_of_val (global_state u x) = Some (instantiate \<Omega> \<tau>')" using state_typ_wf_def by blast
      show  "map_option type_of_val (global_state (update_var \<Lambda>' u d v) x) = Some (instantiate \<Omega> \<tau>')"
...
lemma nstate_old_rel_helper:
  assumes "state_typ_wf [] (global_state ns) (fst \<Lambda>1)" and
          "global_state ns = old_global_state ns" and
          "set (map fst (fst \<Lambda>1)) \<inter> set (map fst (snd \<Lambda>1)) = {}"
          "u \<in> initial_set  R \<Lambda>1 \<Lambda>2 [] ns" and
...
lemma nstate_old_rel_states_helper:
  assumes "state_typ_wf [] (global_state ns) (fst \<Lambda>1)" and
          ...
          "\<And>x y. R_old x = Some y \<longrightarrow> map_of (fst \<Lambda>1) x \<noteq> None" and
          "\<And>x y. R_old x = Some y \<longrightarrow> R x = Some y"
        shows "nstate_old_rel_states \<Lambda>1 \<Lambda>2 R_old ns (initial_set  R \<Lambda>1 \<Lambda>2 [] ns)"
...
lemma rel_well_typed_state_typ_wf: 
  assumes S1:"state_typ_wf \<Omega> (local_state ns) (snd \<Lambda>)" and
        ...
        shows "rel_well_typed \<Lambda> \<Omega> R ns"
  ...
  using state_typ_wf_lookup[OF S1 S2] RelWtVar
   apply blast
  using RelWtConst
  using \<open>\<And>x \<tau>. lookup_var_ty \<Lambda> x = Some \<tau> \<Longrightarrow> \<exists>v. lookup_var \<Lambda> ns x = Some v \<and> type_of_val v = instantiate \<Omega> \<tau>\<close> by force
...
lemma axiom_assm_aux:
  assumes "axiom_assm \<Gamma> consts ns1 axioms" and
        ...
        shows "axiom_assm \<Gamma> consts ns2 axioms"
  using assms
...
proof -
  ...
  show "axioms_sat (consts, []) \<Gamma> (global_to_nstate (\<lambda>x. if map_of consts x \<noteq> None then global_state ns2 x else None)) axioms"
    apply (subst Aux)
    using assms(1)
    unfolding nstate_global_restriction_def state_restriction_def
    apply assumption
